...
public class JsonServiceClient implements ServiceClient
{
	...
	{
		return new GsonBuilder().registerTypeAdapter(Date.class, JsonSerializers.getDateSerializer()).registerTypeAdapter(Date.class,
				JsonSerializers.getDateDeserializer()).registerTypeAdapter(TimeSpan.class, JsonSerializers.getTimeSpanSerializer())
				.registerTypeAdapter(TimeSpan.class, JsonSerializers.getTimeSpanDeserializer()).registerTypeAdapter(UUID.class,
						JsonSerializers.getGuidSerializer()).registerTypeAdapter(UUID.class, JsonSerializers.getGuidDeserializer());
	}
	...
	public <TResponse> TResponse send(HttpURLConnection req, Object responseClass)
	{
		try
		{
			...
			if(Log.isDebugEnabled())
			{
				...
				TResponse response = resClass != null ? (TResponse) getGson().fromJson(json, resClass) : (TResponse) getGson().fromJson(json,
						resType);
			...
			else
			{
				...
				TResponse response = resClass != null ? (TResponse) getGson().fromJson(reader, resClass) : (TResponse) getGson().fromJson(reader,
						resType);
	...
	{
		return relativeOrAbsoluteUrl.startsWith("http:") || relativeOrAbsoluteUrl.startsWith("https:") ? relativeOrAbsoluteUrl : Utils.combinePath
	...
	{
		return send(createRequest(createUrl(request), HttpMethods.Get), request.getResponseType());
	}

	@Override
	public <TResponse> TResponse get(IReturn<TResponse> request, Map<String, String> queryParams)
	{
		return send(createRequest(createUrl(request, queryParams), HttpMethods.Get), request.getResponseType());
	}

	@Override
	public <TResponse> TResponse get(String path, Class responseType)
	{
		return send(createRequest(resolveUrl(path), HttpMethods.Get), responseType);
	}
	...
	{
		return send(createRequest(Utils.combinePath(replyUrl, typeName(request)), HttpMethods.Post, request), request.getResponseType());
	}

	@Override
	public <TResponse> TResponse post(String path, Object request, Class responseType)
	{
		return send(createRequest(resolveUrl(path), HttpMethods.Post, request), responseType);
	}
	...
	{
		return send(createRequest(resolveUrl(path), HttpMethods.Post, requestBody, contentType), responseType);
	}
	...
	{
		return send(createRequest(Utils.combinePath(replyUrl, typeName(request)), HttpMethods.Put, request), request.getResponseType());
	}

	@Override
	public <TResponse> TResponse put(String path, Object request, Class responseType)
	{
		return send(createRequest(resolveUrl(path), HttpMethods.Put, request), responseType);
	}
	...
	{
		return send(createRequest(resolveUrl(path), HttpMethods.Put, requestBody, contentType), responseType);
	}
	...
	{
		return send(createRequest(createUrl(request), HttpMethods.Delete), request.getResponseType());
	}

	@Override
	public <TResponse> TResponse delete(IReturn<TResponse> request, Map<String, String> queryParams)
	{
		return send(createRequest(createUrl(request, queryParams), HttpMethods.Delete), request.getResponseType());
	}

	@Override
	public <TResponse> TResponse delete(String path, Class responseType)
	{
		return send(createRequest(resolveUrl(path), HttpMethods.Delete), responseType);
	}
